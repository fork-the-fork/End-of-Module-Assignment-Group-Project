...
class ClientServerFunctions(unittest.TestCase):
    ...
    def test_large_dictionary_transfer_xml(self):
        self.client.open()
        resp, growth = self._determine_directory_growth(self.client.send_dictionary,
                                                        {f"A{i}": str(i+1) for i in range(10000)},
                                                        "xml")
    ...
    def test_small_dictionary_transfer_xml(self):
        self.client.open()
        resp, growth = self._determine_directory_growth(self.client.send_dictionary,
                                                        {f"A{i}": str(i+1) for i in range(2)},
                                                        "xml")
        self.assertGreater(growth, 0)
        self.assertEqual(resp, 0)
        self.client.close()
    ...
    def test_client_demo(self):
        _, growth = self._determine_directory_growth(client.main)
